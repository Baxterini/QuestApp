...
# --- USER PROFILE (imię + cele/questy) ---
data.setdefault("user", {})
data["user"].setdefault("name", "")
data["user"].setdefault("goals", [])  # np. ["Mind", "Sport"]
save_data(data)
...
with st.container():
    ...
    goals = st.multiselect(
        "Co chcesz poprawić?",
        options=QUEST_TOPICS,
        default=data["user"]["goals"],
        help="Wybierz 1–3 na start. Zawsze możesz zmienić później."
    )
    ...
    with col_b:
        if st.button("🔄 Wyczyść"):
            ...
            data["user"]["goals"] = []
            save_data(data)
            st.info("Wyczyszczono.")
            st.rerun()

# Powitanie
if data["user"]["name"]:
    st.caption(f"👤 Witaj, **{data['user']['name']}**! Twoje questy: {', '.join(data['user']['goals']) or '—'}")
else:
    st.caption("👤 Dodaj imię i wybierz questy, żeby spersonalizować widok (opcjonalnie).")
